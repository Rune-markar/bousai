...
export default function PracticalLoadout({ taskId, state, onChange, onBagSettings, onComplete, onClose }) {
  ...
  return <div className="loadout-backdrop" role="presentation" onMouseDown={(event) => event.target === event.currentTarget && onClose()}>
    <section ref={dialogRef} className="loadout-modal" role="dialog" aria-modal="true" aria-labelledby="loadout-title" style={{ '--loadout-accent': loadout.accent }}>
      <header className="loadout-head"><div><span>{loadout.label}</span><h2 id="loadout-title">{loadout.title}</h2><p>{loadout.subtitle}</p></div><button ref={closeRef} type="button" aria-label="装備ケースを閉じる" onClick={onClose}><X /></button></header>
      ...
      <div className={settings ? 'bag-planning-layout' : ''}>
        {settings && <section className="inventory-packer" aria-labelledby="auto-pack-title">
          <header className="packer-head"><div><span>AUTO IDENTIFIED・{packing.profile.stageLabel}</span><h3 id="auto-pack-title">保有備蓄から自動提案</h3><p>左に現状の在庫、右にバッグへ提案する中身を同時表示します。</p></div><b>{packing.items.length}<small>品目を提案</small></b></header>
          ...
          <div className="volume-examples" aria-label="容量の比較例">{BAG_VOLUME_EXAMPLES.map((item) => <span key={item.label}>{item.symbol}<b>{item.label}</b><small>約{item.volumeMl}ml</small></span>)}</div>
          <div className="packing-split">
            <section><h4>現状の備蓄品</h4><div className="owned-stock-list">{owned.length ? owned.map((item) => { const proposed = proposedById.get(item.id); return <article key={item.id}><span>{itemSymbol(item.category)}</span><div><b>{item.name}</b><small>在庫 {item.quantity}{item.unit}</small></div>{proposed ? <em>バッグへ {proposed.quantity}{proposed.unit}</em> : <i>在庫のみ</i>}</article>; }) : <p>登録済みの備蓄品がありません。</p>}</div></section>
            <section><h4>バッグへの自動配置</h4><div className="auto-pack-list">{packing.items.length ? packing.items.map((entry) => <article key={entry.id}><span className="pack-item-icon">{itemSymbol(entry.category)}</span><div><b>{entry.name}</b><small>{entry.quantity}{entry.unit}・{(entry.totalMl / 1000).toFixed(2)}L</small></div><em className={entry.volumeSource === 'user' ? 'measured' : ''}>{entry.reason}・{entry.volumeLabel}</em></article>) : <p className="auto-pack-empty">容量内に選定できる登録済み備蓄がありません。</p>}</div></section>
          </div>
        ...
        <section className="ideal-loadout-panel" ref={checklistRef} tabIndex="-1">
          <header><div><span>IDEAL REFERENCE</span><h3>理想構成を参考に確認</h3></div>{loadout.referenceBudget && <div className="loadout-budget"><small>参考予算（アプリ内概算）</small><b>{loadout.referenceBudget.range}</b><span>{loadout.referenceBudget.note}</span></div>}</header>
          <div className="loadout-workbench">
            <div className="loadout-case" role="group" aria-label={`${loadout.title}の装備品`}>{loadout.items.map((item) => { const packed = status.packed.has(item.id); return <button type="button" key={item.id} className={`loadout-item ${packed ? 'packed' : ''} ${selected?.id === item.id ? 'selected' : ''}`} onClick={() => setSelectedId(selected?.id === item.id ? '' : item.id)} aria-label={`${item.name}の詳細を表示`} aria-expanded={selected?.id === item.id}><span className="item-symbol" aria-hidden="true">{item.symbol}</span><span className="item-check">{packed && <Check />}</span></button>; })}</div>
