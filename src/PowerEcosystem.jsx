...
export default function PowerEcosystem({ plan, onChange, onBack }) {
  ...
  return <section className="power-page power-ecosystem" aria-labelledby="power-page-title">
    ...
    <div className="power-tabs" role="tablist" aria-label="電力設計の項目">
      {tabs.map((tab) => <button key={tab.id} id={`power-tab-${tab.id}`} type="button" role="tab" aria-selected={activeTab === tab.id} aria-controls={`power-panel-${tab.id}`} tabIndex={activeTab === tab.id ? 0 : -1} onClick={() => setActiveTab(tab.id)}>{tab.label}</button>)}
    </div>

    <div className="power-tab-viewport">
      {activeTab === 'devices' && <section id="power-panel-devices" role="tabpanel" aria-labelledby="power-tab-devices" aria-label="機器">
      ...
      {activeTab === 'battery' && <section id="power-panel-battery" role="tabpanel" aria-labelledby="power-tab-battery" aria-label="蓄電池">
      ...
      {activeTab === 'solar' && <section id="power-panel-solar" role="tabpanel" aria-labelledby="power-tab-solar" aria-label="太陽光">
        <div className="power-summary-cards">
...
function HelpSheet({ help, result, updateDevice, onClose }) {
  ...
  return <div className="modal-backdrop power-backdrop" onMouseDown={(event) => event.target === event.currentTarget && onClose()}>
    <section className="power-modal power-help-sheet" role="dialog" aria-modal="true" aria-labelledby="power-help-title" data-help-id={help.id}>
