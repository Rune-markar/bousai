...
const nodes = [
  ...
  { id: 'diversity-power', title: '停電時の選択肢を足す', kind: 'diversity', category: 'light', state: 'locked', condition: '必要な電源を登録する' },
];
...
describe('StockpileSkillTree', () => {
  ...
  it('横並びノードを使わず、家庭の3資源と右側の達成ラインに整理する', () => {
    ...
    expect(screen.getByRole('region', { name: 'わが家の備蓄達成ライン' })).toBeInTheDocument();
    expect(screen.getByRole('heading', { name: '3つを同じラインまでそろえる' })).toBeInTheDocument();
    expect(screen.getByRole('button', { name: '飲料水、現在3日分、確認できます' })).toBeInTheDocument();
    expect(screen.getByRole('button', { name: '食料（重量換算）、現在2日分、未達成' })).toBeInTheDocument();
    expect(screen.getByRole('button', { name: '携帯トイレ、現在3日分、再確認' })).toBeInTheDocument();
    ...
    const lineList = screen.getByRole('complementary', { name: '達成ライン一覧' });
    expect(within(lineList).getByRole('button', { name: '3日ライン、2/3項目到達、未達成' })).toBeInTheDocument();
    expect(within(lineList).getByRole('button', { name: '7日ライン、0/3項目到達、未達成' })).toBeInTheDocument();
    expect(within(lineList).getByText('3項目が同じ線へ届くと達成')).toBeInTheDocument();
    expect(container.querySelector('.stockpile-skill-tree-content')).toHaveAttribute('data-no-horizontal-scroll', 'true');
    expect(container.querySelector('.stockpile-skill-tree-connectors')).not.toBeInTheDocument();
  });

  it('安全・家族固有・暮らしの継続を量から分割し、具体名を表示する', () => {
    render(<StockpileSkillTree nodes={nodes} onClaim={onClaim} onClose={onClose} />);

    expect(screen.getByRole('heading', { name: '住まい・連絡・薬' })).toBeInTheDocument();
    expect(screen.getByRole('heading', { name: 'わが家の個別用品' })).toBeInTheDocument();
    expect(screen.getByRole('heading', { name: '電源・食の幅・休息' })).toBeInTheDocument();
  ...
  it('資源カードのタップで条件を表示し、明示ボタンから達成を確定できる', () => {
    ...
    fireEvent.click(screen.getByRole('button', { name: '飲料水、現在3日分、確認できます' }));
    ...
    fireEvent.click(screen.getByRole('button', { name: '食料（重量換算）、現在2日分、未達成' }));
  ...
  it('650msの長押しでclaimし、pointer cancelと大きな移動では中止する', () => {
    vi.useFakeTimers();
    render(<StockpileSkillTree nodes={nodes} onClaim={onClaim} onClose={onClose} />);
    const water = screen.getByRole('button', { name: '飲料水、現在3日分、確認できます' });
  ...
  it('Escapeで閉じ、未達成ラインでも条件を確認できる', () => {
    render(<StockpileSkillTree nodes={nodes} onClaim={onClaim} onClose={onClose} />);
    fireEvent.click(screen.getByRole('button', { name: '3日ライン、2/3項目到達、未達成' }));
  ...
  it('単独のmodelノードも説明とprogressを表示できる', () => {
    ...
    fireEvent.click(screen.getByRole('button', { name: '飲料水、現在1日分、確認できます' }));
    expect(screen.getByRole('heading', { name: '飲料水' })).toBeInTheDocument();
