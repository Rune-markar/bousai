export const POWER_DEVICES = [
  ...
  { id: 'fan', symbol: '🌀', name: '扇風機', watts: 25, hours: 8, defaultQuantity: 1, requiresSurgeConfirmation: true, note: '省電力モデルの弱〜中運転。起動電力は実機仕様を確認' },
  ...
  { id: 'cpap', symbol: '🫁', name: 'CPAP等医療機器', watts: 40, hours: 8, defaultQuantity: 0, requiresSurgeConfirmation: true, note: '実機ラベルと医療者の指示を優先' },
  { id: 'fridge', symbol: '🧊', name: '小型冷蔵庫', watts: 60, hours: 12, defaultQuantity: 0, requiresSurgeConfirmation: true, note: '間欠運転の平均。起動電力は実機仕様を確認' },
];
  ...
  return {
    ...
    devices: Object.fromEntries(POWER_DEVICES.map((device) => {
      ...
      return [device.id, {
        ...
        surgeWatts: clamp(source.surgeWatts, 0, 10000, 0),
...
export function calculatePowerSystem(rawPlan = {}) {
  ...
  const rows = POWER_DEVICES.map((device) => {
    ...
    const runningWatts = plan.mode === 'detail' && setting.actualWatts > 0 ? setting.actualWatts : setting.expectedWatts;
    const surgeConfirmed = !device.requiresSurgeConfirmation || setting.surgeWatts >= runningWatts;
    const instantaneousWatts = setting.surgeWatts > 0 ? Math.max(runningWatts, setting.surgeWatts) : runningWatts;
    const dailyWh = runningWatts * setting.hours * setting.quantity;
    return { ...device, ...setting, runningWatts, surgeConfirmed, instantaneousWatts, dailyWh };
  });
  const selected = rows.filter((row) => row.quantity > 0);
  const dailyLoadWh = selected.reduce((sum, row) => sum + row.dailyWh, 0);
  const runningLoadW = selected.reduce((sum, row) => sum + row.runningWatts * row.quantity, 0);
  const unconfirmedSurgeDevices = selected.filter((row) => !row.surgeConfirmed);
  const outputSizingComplete = unconfirmedSurgeDevices.length === 0;
  const surgeLoadW = outputSizingComplete
    ? selected.reduce((sum, row) => sum + row.instantaneousWatts * row.quantity, 0)
    : null;
  ...
  const recommendedContinuousOutputW = runningLoadW ? roundUp(runningLoadW * 1.25, 100) : 0;
  const recommendedSurgeOutputW = surgeLoadW === null
    ? null
    : surgeLoadW ? roundUp(surgeLoadW * 1.25, 100) : 0;
  const recommendedOutputW = outputSizingComplete
    ? Math.max(recommendedContinuousOutputW, recommendedSurgeOutputW)
    : null;
  ...
  return {
    ...
    runningLoadW: Math.round(runningLoadW),
    surgeLoadW: surgeLoadW === null ? null : Math.round(surgeLoadW),
    peakLoadW: surgeLoadW === null ? null : Math.round(surgeLoadW),
    unconfirmedSurgeDevices,
    outputSizingComplete,
    ...
    requiredBatteryWh,
    requiredSolarW,
    recommendedContinuousOutputW,
    recommendedSurgeOutputW,
    recommendedOutputW,
    batteryYenPer10Wh,
    solarYenPerW,
    batteryEstimateYen,
